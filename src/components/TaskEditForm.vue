<template>
    <form @submit.prevent="taskStore.updateTask">
        ...
        <label for="taskName" class="text-gray-800 block text-center py-2 dark:text-white">
            ...
            <input type="text" v-model="taskStore.editingTask.name" id="taskName" class="text-gray-800 block mx-auto border-2 border-gray-300 rounded px-2 min-w-[260px] sm:w-[400px] md:w-[500px] lg:w-[540px]">
        </label>
        <label for="taskDetail" class="text-gray-800 block text-center py-2 dark:text-white">
            <span class="font-titan">Detail</span>
            <textarea v-model="taskStore.editingTask.detail" id="taskDetail" class="text-gray-800 block mx-auto rounded min-w-[260px] h-40 border-2 border-gray-300 p-2 sm:w-[400px] md:w-[500px] lg:w-[540px] sm:h-40 md:h-40 lg:h-40">
            </textarea>
        </label>
        ...
        <div class="flex justify-between mx-auto py-4 w-full sm:w-[300px] md:w-[300px] lg:w-[380px] font-titan">
            <button type="button" @click="taskStore.closeModal" class="px-4 py-2 bg-gray-600 rounded min-w-[100px] hover:text-orange-400 font-titan">
                Cancel
            </button>
            <button type="submit" class="px-4 py-2 bg-gray-600 rounded min-w-[100px] hover:text-orange-400">
                Update
            </button>
        </div>
